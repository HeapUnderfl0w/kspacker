...
impl Packer {
    ...
    pub fn collect(&self, allow_builtin: bool) -> Result<PackablePreset, PackError> {
        ...
        self.get_textures(
            &mut files,
            &loaded_preset.effects.keypresses.keypress_material,
        );
        self.get_textures(
            &mut files,
            &loaded_preset.effects.note_objects.note_border_material,
        );
        self.get_textures(
            &mut files,
            &loaded_preset.effects.note_objects.note_object_material,
        );
    ...
    fn make_found_file(&self, typ: TextureType, file: &str) -> FoundAsset {
        fn test_exts(p: &PathBuf, n: &str) -> Option<(PathBuf, &'static str)> {
        ...
        let pathinfo = test_exts(
            &helpers::root_asset_dir(&self.root).join(typ.path_name()),
            file,
        );
        ...
        let pathinfo = test_exts(
            &helpers::custom_asset_dir(false).join(typ.path_name()),
            file,
        );
        if let Some((path, ext)) = pathinfo {
    ...
    fn check_builtin_preset(&self) -> bool {
        helpers::root_preset_dir(&self.root)
            .join(format!("{}.json", self.preset))
            .exists()
    }
}
...
impl PackablePreset {
    pub fn pack(self, to: &str, extra_meta: ExtraMeta) -> Result<(), PackError> {
        ...
        for asset in self
            .assets
            .into_iter()
            .filter(|asset| asset.action == AssetAction::Pack)
        {
            let mut src =
        ...
        let meta = PackMetaData {
            name:           extra_meta.rename.unwrap_or(self.name),
